...
packagename::functionname()</code></pre>
<!-- CODE (show) -->
...
#&#x2060;#&#x2060; 3 Premium 2008    326
#&#x2060;#&#x2060; 4 Premium 2009    332
#&#x2060;#&#x2060; 5 Good    2008    237</code></pre></section>
<!-- CODE (hide) -->
...
We are going to that by importing, wrangling and visualizing of the provided data.
...
In the console you see, that `read_csv()` detects automatically the datatype of each column. In your environment you should see now 4 loaded tables. You can click on them to take a look at the data.
...
#&#x2060;#&#x2060; $ product.width.cm              &lt;dbl&gt; 14, 40, 33, 15, 30, 15, 20, 45, 1…
#&#x2060;#&#x2060; $ seller.zip.code.prefix        &lt;chr&gt; "27277", "03471", "37564", "14403…
#&#x2060;#&#x2060; $ seller.location               &lt;chr&gt; "volta redonda, SP", "sao paulo, …</code></pre>
...
* Take a look at the colum category names (e.g. by running `order_items_joined_tbl$product.category.name`). Some entries seem to have a main and a sub product category separated by a `-`. For example there are furnitures - office (moveis - escritorio) and furnitures - kitchen_dining_laundry_garden (moveis - cozinha_area_de_servico_jantar_e_jardim). You can print all unique entires, that contain `moveis` with the following code chunk:
...
Now that we got the data in a good format, we can start to create business insights for that. We are going to do two analyses. The first is Sales by year and the second is sales by category. This will be a 2 step process: Step 1: Manipulate / prepare the data and Step 2: Visualize it (this is one of the most important skills a data scientist needs to know). This section will give you just some exposure to visualization. Chapter 4 will be completely dedicated to this topic.

* select needed cols
* get years
* group by years and summarize sales
...
<div id="container">
  <div id="first">{{% icon download %}}</div>
  <div id="second"><a href="" target="_blank"><b>code_checkpoint_01.R</b></a></div>
  <div id="clear"></div>
</div>
...
Analyze the sales by location of the sellers. You will need to import the following table. Since `state` and `city` are multiple features (variables), they should be split.
